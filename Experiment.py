# -*- coding: utf-8 -*-
"""
Created on Sun Mar  3 20:24:56 2024
...
def average_over_repetitions(backup, n_repetitions, n_timesteps, max_episode_length, learning_rate, gamma, policy='egreedy', 
    ...
    for rep in range(n_repetitions): # Loop over repetitions
        if backup == 'q':
            ...
            returns, timesteps = sarsa(n_timesteps, learning_rate, gamma, eval_interval, policy, epsilon, temp, plot)
        elif backup == 'nstep':
...
def experiment():
    ...
    # Experiment      
    n_repetitions = 20
    smoothing_window = 5 # Must be an odd number. Use 'None' to switch smoothing off!
    plot = False # Plotting is very slow, switch it off when we run repetitions
    
    # MDP    
    n_timesteps = 30001 # Set one extra timestep to ensure evaluation at start and end
    eval_interval = 3000
    ...
    Plot.save(r'E:\2024 Sem2\RL\Assignment\exploration.png')
    ...
    Plot.add_hline(optimal_episode_return, label="DP optimum")
    Plot.save(r'E:\2024 Sem2\RL\Assignment\on_off_policy.png')
    ...
    Plot.add_hline(optimal_episode_return, label="DP optimum")
    Plot.save(r'E:\2024 Sem2\RL\Assignment\depth.png')'''
